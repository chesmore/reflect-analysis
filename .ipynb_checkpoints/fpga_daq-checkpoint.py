...
def refl_save(plate, sample, N_MULT):
    ...
    while i < nsamp:
        ...
        arr_AA[np.abs(arr_AA) == 0.0] = replacement_val
        arr_BB[np.abs(arr_BB) == 0.0] = replacement_val
        arr_AB[np.abs(arr_AB) == 0.0] = replacement_val
        arr_AA_SAMPL[np.abs(arr_AA_SAMPL) == 0.0] = replacement_val
        arr_BB_SAMPL[np.abs(arr_BB_SAMPL) == 0.0] = replacement_val
        arr_AB_SAMPL[np.abs(arr_AB_SAMPL) == 0.0] = replacement_val
        ...
        arr_R_cross = np.divide(
            (arr_AB_SAMPL / arr_AA_SAMPL), (arr_AB / arr_AA)
        )  # Cross correlated signal
        arr_R_receive = np.divide(
            arr_BB_SAMPL, arr_BB
        )  # Cross correlated signal at receiver
        arr_R_source = np.divide(
            arr_AA_SAMPL, arr_AA
        )  # Cross correlated signal at soure
